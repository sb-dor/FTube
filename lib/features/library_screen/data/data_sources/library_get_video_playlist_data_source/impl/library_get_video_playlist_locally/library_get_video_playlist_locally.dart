...
class LibraryGetVideoPlaylistLocally implements LibraryGetVideoPlaylistDataSource {
  ...
  Future<PlaylistModelDb?> videoPlaylist(BaseVideoModelDb? baseVideoModelDb) async {
    ...
    final findVideoFirst = await _dbFloor.playListDao.getVideoFromPlaylistVideos(
      baseVideoModelDb.videoId ?? '0',
    );

    final result = await _dbFloor.playListDao.getVideoPlaylist(findVideoFirst?.playlistId ?? 0);
